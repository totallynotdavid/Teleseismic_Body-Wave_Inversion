...
*     du=duration
         READ(5,*) ta,du,dt,f1,f2,iph,nr
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      open(1,file=fname)
C     READ(1,'(g15.7)') dt0
C     read(1,'(/////5g15.7)') dum, alat, alon, elev
      READ(1,*) dt0
      read(1,'(////)')
      read(1,*) dum, alat, alon, elev
C
      read(1,'(///////5i10)') yr, nday, hr,mnu, nsec
      read(1,'(5i10)') nmsec,ndum,ndum,ndum,nd
       if(nd.gt.nd0) print *,nd
      read(1,'(/////)')
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      open(13,file='iris.new.response')
112     READ(13,*,end=199)
       read(13,'(13x,a4)',end=199) name0
       read(13,*)
       read(13,'(13x,a3)') comp0
       read(13,'(///31x,f25.0)') A0
       read(13,*)
       read(13,'(30x,a)') charac
       read(charac,*) nzero1
       read(13,'(30x,a)') charac
       read(charac,*) npole1
       read(13,*)
       ...
       if(index(name,name0).eq.0) goto 113
       if(index(comp,comp0).eq.0) goto 113
         zero(n)=cmplx(a1,a2)
113    continue
       read(13,*)
       read(13,*)
       do 114 n=1,npole1
       read(13,*) idum,a1,a2,a3,a4
       if(index(name,name0).eq.0) goto 114
       if(index(comp,comp0).eq.0) goto 114
         zpole(n)=cmplx(a1,a2)
114    continue
       read(13,'(31x,f25.0)') Sens 
       if(index(name,name0).eq.0) goto 112
       if(index(comp,comp0).eq.0) goto 112
        nzero=nzero1+1
        npole=npole1
	zero(nzero)=0.
       Sc=1.e+6/(A0*Sens)
...
*     call jbtravel(delta,Depth,Tp,Ts)
...
C   Taper
C       do 15 i=1,nd/20
C       tape=0.5*(1+cos(pi*(i-1.)*20./nd))
C15      y(nd-nd/20+i)=tape*y(nd-nd/20+i)
C
       nn=log(nd*1.)/log(2.)
       nn=2**nn
       if(nn.lt.nd) nn=nn*2
...
334    zt=zt/(zjw-zero(n))
        ZC=ZC1*ZC2*zt
C       zc=zt
       if(id.eq.1) zc=zc*cmplx(0,2*pi*f)
       Z(I)=Z(I)*ZC
...
*  hr0:mnu0:sec0=origin time
        ...
        sec2=sec2-mnu2*60
        mnu2=mnu0+mnu2
        write(22,201) name,mnu2,sec2,-ta,f1,f2
201     format(a5,i4,f7.2,f8.2,3f7.2)
        isft=ta/dt+.5
